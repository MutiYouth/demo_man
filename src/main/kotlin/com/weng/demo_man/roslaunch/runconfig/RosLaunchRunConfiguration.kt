...
import com.weng.demo_man.cli.RosCommandLineState
...
class RosLaunchRunConfiguration(project: Project, configurationFactory: ConfigurationFactory, targetName: String) :
    ...
    override fun getState(executor: Executor, exeEnv: ExecutionEnvironment) =
        RosCommandLineState(
            exeEnv, RosConfig.rosSettings.localRos.shell.name, "-c",
            RosConfig.rosSettings.localRos.launch(
                launchFile = path?.path ?: "",
                customEnv = mapOf(
                    "ROS_MASTER_URI" to "http://$rosMasterAddr:$rosMasterPort",
                    "PYTHONUNBUFFERED" to "1"
                ), options = getParameters(), args = ""
            ).toString()
        )
