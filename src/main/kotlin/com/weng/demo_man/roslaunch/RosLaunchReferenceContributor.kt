...
class RosLaunchReferenceContributor : PsiReferenceContributor() {
    ...
    private fun matcher(pattern: String) =
        XmlPatterns.xmlAttributeValue().withValue(StandardPatterns.string().matches(pattern))!!

    private fun PsiReferenceRegistrar.bind(
        pattern: String,
        reference: (XmlAttributeValue) -> PsiReferenceBase<XmlAttributeValue>
    ) =
