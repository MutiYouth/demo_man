package com.weng.demo_man.codeglance.ui
...
class DonationDialog : DialogWrapper(null) {
	...
	private fun createDonatePanel(): JPanel {
		return NonOpaquePanel(MigLayout()).apply {
			add(LinkLabel(null, CodeGlanceIcons.loadRoundImageIcon("/config_data/inte_cgp/image/paypal.png")) { _: LinkLabel<Any?>, _: Any? ->
			...
			val weChatPayLabel = JBLabel(localMessage("donate.wechat")).apply {
				...
				icon = CodeGlanceIcons.scaleIcon(CodeGlanceIcons.loadImageIcon("/config_data/inte_cgp/image/wechat_pay.png"), 0.5f)
			}
			add(weChatPayLabel)
			val aliPayLabel = JBLabel(localMessage("donate.alipay")).apply {
				horizontalTextPosition = SwingConstants.CENTER
				verticalTextPosition = SwingConstants.BOTTOM
				icon =  CodeGlanceIcons.scaleIcon(CodeGlanceIcons.loadImageIcon("/config_data/inte_cgp/image/ali_pay.png"), 0.5f)
			}
